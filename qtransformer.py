...
class FeedForwardQuantum(FeedForwardBase):
    def __init__(self, embed_dim, n_qubits, n_qlayers=1, dropout=0.1, q_device="default.qubit", ansatz_id=1):
...
class TransformerBlockQuantum(TransformerBlockBase):
    def __init__(self,
        ...
        self.attn = MultiHeadAttentionQuantum(embed_dim,
                                              ...
                                              ansatz_id=ansatz_id)
        if n_qubits_ffn > 0:
            self.ffn = FeedForwardQuantum(embed_dim, n_qubits_ffn, n_qlayers, q_device=q_device, ansatz_id=ansatz_id)
        else:
        ...
        if n_qubits_transformer > 0:
            ...
            transformer_blocks = [
                TransformerBlockQuantum(embed_dim, num_heads, ffn_dim,
                                        ...
                                        n_qlayers=n_qlayers, 
                                        q_device=q_device,
                                        ansatz_id=ansatz_id) for _ in range(num_blocks)
            ]
        else:
            transformer_blocks = [
